...
| 市场观察 | `MarketInsightPanel.tsx`、`InvestmentValueMetrics.tsx` | 确定性指标、公告/要闻、客观事件，以及 PE TTM/PB历史与行业分位、自由现金流、ROIC和净负债；金融企业显示不适用 | 实时行情 + schema v3 基本面快照 + 按需估值历史 + 独立 `market-insight` 模块 |
