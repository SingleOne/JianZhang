...
`DailyMarketScanService` 只为活跃标的获取 24 根前复权日 K，扫描队列固定最多 8 个并发，不进入详情页 `KlineHub` 的全局串行队列。两条链路共享 `HistoricalKlineCache`：扫描优先命中已有日 K，网络结果也写回同一缓存。单股失败只计入失败数量，不中断其余标的。
...
- 长上影线：上影线不短于实体，且上影线占当日最高价与最低价之差至少 40%。
- 长下影线：下影线不短于实体，且下影线占当日最高价与最低价之差至少 40%。
- 布林带收窄/扩张：按 20 日收盘价均值与 2 倍标准差计算上下轨；最近 4 次交易日带宽环比必须连续同向，且首尾累计变化绝对值至少 10%。

### 追踪量比

追踪中的股票由主进程读取日 K，分别计算“当日成交量 ÷ 前 5/10/20 个交易日平均成交量”，并在同一每日快照保存收盘价、涨跌幅、成交量、成交额、5/10/20 日均价、均量、周期涨幅、长影线和连续布林带状态。启动、新增追踪股票后会立即补取，运行期间每 30 分钟更新；每个交易日只保留一条通用指标快照，同日新结果覆盖对应指标字段，未来新增的其他指标不受影响。日 K 最多补取 500 条，因此应用未运行期间的追踪日期也可在下次启动时回填。停止追踪后不再新增快照，已有量价、量比和技术形态趋势永久保留。
...
基本面数据不进入实时行情调度。`scripts/generate_fundamental_snapshot.py` 按五个阶段读取东方财富数据中心公开接口：
...
5. `RPT_DMSK_FN_INCOME`、`RPT_DMSK_FN_CASHFLOW`、`RPT_DMSK_FN_BALANCE`和`RPT_F10_FINANCE_GBALANCE`：保存最近八个季度的累计/单季度经营现金流、应收收入背离、存货周转天数变化和商誉占比等财务排雷数据。

新生成结果使用 schema v7，并继续兼容读取 schema v1/v2/v3/v4/v5/v6。v5在估值快照中增加同一交易日收盘价，结合总市值反推总股本；v6增加季度财务排雷序列；v7增加 PCF TTM、同行分位及覆盖率。生产安装包不内置完整快照；`FundamentalDataService` 启动时读取并缓存 `userData/fundamentals/snapshot.json`，文件不存在时自动把首次获取任务加入共享 Python 队列。快照超过 90 天、完整财年落后或仍为缺少 DCF、季度排雷或 PCF 行业分位数据的旧版 schema 时，只在“基本面初筛”和“设置 → 系统与数据”提示并提供手动更新，不会自动刷新。

市盈率 TTM使用东方财富主行情字段，市净率使用同一行情链路；切换到腾讯备用行情时读取腾讯对应字段，新浪备用源不提供这两项估值。“基本面”和“市场观察”复用同一估值与资本回报组件，展示实时 PE/PB、按当前总市值与最近连续四季度经营现金流计算的 PCF TTM、schema v7快照日行业分位、总市值、流通市值及 `RPT_VALUEANALYSIS_DET` 近五年日度历史分位，历史序列按股票缓存于 `market-cache/valuations/`。行情源未给出总市值时，按快照日总市值和收盘价反推总股本后乘当前价。最近四个季度不连续、经营现金流不为正或属于银行、证券、保险时不发布 PCF。PCF/PE 小于 0.9 表示现金含金量较高，0.9—1.1 视为基本匹配，大于 1.1 提醒现金滞后；最近三个完整财年按“归母净利润÷经营现金流”得到的同口径比值均不低于 1.5 时，标记红色“多年剪刀差”。行情时间、财报截止期、五年财务年度、历史区间和行业估值日期分别标注。
...
同一文件还通过 `evaluateFundamentalQuality` 计算固定质量标签，不随界面临时筛选条件变化。严格基本面、现金持续、利润成长、ROE稳定和扣非扎实只作为默认三项通过公司的增强特征；“现金含金量”要求最近三个完整财年按“归母净利润÷经营现金流”得到的 PCF/PE 均低于 0.9；改善观察要求最近三年加权 ROE 与净利润严格连续增长，且最新一年现金转换率严格大于 100%。标签允许重叠，不合成分数；金融企业和数据不完整项目不生成相应标签。

`evaluateFundamentalRisk` 使用同一快照生成八类固定风险提示：现金背离（五年加权 ROE 每年大于 15%，但五年累计现金转换率低于 80%）、高杠杆 ROE（五年加权 ROE 每年大于 15%，行业负债百分位不低于 80%）、扣非偏弱（五年加权 ROE 每年大于 15%，扣非 ROE 完整且至少一年不高于 15%）、利润现金背离（最近三年利润为正并连续增长，经营现金流连续下降，且最新现金转换率低于 100%）、PCF高于PE（最新完整财年 PCF/PE 高于 1.1）、多年剪刀差（最近连续三个完整财年 PCF/PE 均不低于 1.5）、ROE 下滑（五年加权 ROE 每年大于 15%，最新值较五年前下降至少 5 个百分点）和单年现金转弱（五年累计现金转换率大于 100%，但最新一年低于 100%）。现金背离、利润现金背离和多年剪刀差属于红色严重风险，其余属于橙色关注项；达到多年剪刀差时不重复生成 PCF高于PE 标签。标签可重叠，不合成分数，也不改变硬筛选结果。金融企业和经营现金流不为正等不足以比较 PCF/PE 的数据不生成对应风险。
...
股东数据不进入报价刷新调度。用户首次打开个股“股东”标签时，`ShareholderService` 将现有 `quoteId` 转为东方财富 F10 使用的 `SH/SZ/BJ + 股票代码`，按需查询股东研究接口。返回结果归一化为实际控制人、最新股东户数与户均持股概览、近十期股东户数、十大股东和十大流通股东；界面分别标注股东报告期和接口获取时间，不把定期披露数据解释为实时持仓。

每次成功更新都按 `quoteId` 写入 `userData/market-cache/shareholders/<市场>_<代码>.json`，例如 `1_600519.json`，缓存有效期为 24 小时。应用重启后优先复用有效磁盘数据；用户可点击“更新数据”绕过有效期。接口失败但存在旧文件时继续返回旧数据并显示刷新警告，没有真实数据时不生成占位股东。浏览器演示环境使用单独的显式演示快照，不会写入桌面缓存。
