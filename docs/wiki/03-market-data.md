...
`dcf-analysis.ts` 只对普通企业生成简化 DCF：以最近三年平均自由现金流为基准，历史自由现金流复合增速限制在 -10%—15% 后预测五年，使用10%折现率和3%永续增长率；企业价值扣除最新净负债后，按快照日总市值与收盘价推算的总股本换算每股估值。基本面页将结果与实时股价比较，同时展示高低幅度和 DCF/现价；后者低于70%时提示当前价格显著高于模型估值。最近三年平均自由现金流非正、净负债或股本换算数据缺失时不强行给值，金融企业明确标为不适用。
