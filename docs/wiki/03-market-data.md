...
新生成结果使用 schema v6，并继续兼容读取 schema v1/v2/v3/v4/v5。v5在估值快照中增加同一交易日收盘价，结合总市值反推总股本；v6增加季度财务排雷序列。生产安装包不内置完整快照；`FundamentalDataService` 启动时读取并缓存 `userData/fundamentals/snapshot.json`，文件不存在时自动把首次获取任务加入共享 Python 队列。快照超过 90 天、完整财年落后或仍为缺少 DCF 股本换算数据的旧版 schema 时，只在“基本面初筛”和“设置 → 系统与数据”提示并提供手动更新，不会自动刷新。

市盈率 TTM使用东方财富主行情字段，市净率使用同一行情链路；切换到腾讯备用行情时读取腾讯对应字段，新浪备用源不提供这两项估值。“基本面”和“市场观察”复用同一估值与资本回报组件，展示实时 PE/PB、schema v6快照日行业分位、总市值、流通市值及 `RPT_VALUEANALYSIS_DET` 近五年日度历史分位，历史序列按股票缓存于 `market-cache/valuations/`。PCF TTM按当前总市值除以最近连续四个季度经营现金流计算；行情源未给出总市值时，按快照日总市值和收盘价反推总股本后乘当前价。最近四个季度不连续、经营现金流不为正或属于银行、证券、保险时不发布 PCF。PCF 当前不计算历史或行业分位。行情时间、财报截止期、五年财务年度、历史区间和行业估值日期分别标注。
