...
目录按股票写入 `userData/company-reports/{code}.json` 并缓存 24 小时，用户点击“更新目录”时绕过有效期重新查询。在线更新失败且存在旧目录时返回已过滤为五年的旧缓存并提示。用户点击“AI 总结”后，主进程只接受 `https://static.cninfo.com.cn/` 的 PDF，下载到内存并用 `pdf-parse` 提取业务概要、主要财务指标、管理层分析、审计报告、三张表和附注等重点片段，再调用 AI Runtime 生成纯文本总结；结果写入 `userData/company-reports/summaries.json`，PDF 不落盘。原文打开 IPC 使用同一域名校验。
