...
`CompanyReportService` 使用巨潮资讯股票组织列表和历史公告接口，按股票分别查询年报、半年报、一季报和三季报分类。查询范围为当前日期向前十个自然年，结果保留报告标题、财年、公告时间、全文/摘要/英文版标记、修订标记和官方 PDF 链接；renderer 默认只显示报告全文，可按类型、财年和标题过滤，也可开启摘要及英文版。

目录按股票写入 `userData/company-reports/{code}.json` 并缓存 24 小时，用户点击“更新目录”时绕过有效期重新查询。在线更新失败且存在旧目录时返回旧缓存并提示，不下载或解析 PDF。原文打开 IPC 只接受 `https://static.cninfo.com.cn/` 链接。
