...
| 可见股票盘口 | 跟随该股票重点/普通间隔，3 秒内复用 | 主进程 `OrderBookHub` + React 面板 |
| 重点股票大单盘口 | 每个重点刷新周期轮询 1 只 | 主进程 `OrderBookHub` |
...
分时、K 线、资金流和板块使用组件文件内的模块级 `Map`。特点：

- 在展开/收起或切换股票后仍可复用。
- 只存在于当前渲染进程内。
- 应用重启后丢失。

盘口缓存已经移到 Electron 主进程的 `OrderBookHub`，分时图、大单提醒、市场观察和做 T 参考共享同一份进行中请求和最近成功结果。
