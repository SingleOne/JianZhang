...
新生成结果使用 schema v2，并继续兼容读取 schema v1。生产安装包不内置完整快照；`FundamentalDataService` 启动时读取并缓存 `userData/fundamentals/snapshot.json`，文件不存在时自动把首次获取任务加入共享 Python 队列。快照超过 90 天或完整财年落后时，只在“基本面初筛”和“设置 → 系统与数据”提示并提供手动更新，不会自动刷新。

市盈率 TTM使用东方财富主行情字段，市净率使用同一行情链路；切换到腾讯备用行情时读取腾讯对应字段，新浪备用源不提供这两项估值。PE和 PB跟随主表报价刷新，不写入基本面快照。个股“市场观察”把实时 PE/PB与最新完整财年的自由现金流、ROIC和净负债放在同一组展示，但分别标明行情时间和财务报告期。
