...
- 当前没有自动下单或券商连接；`ai` 支持 OpenAI API 和 DeepSeek API，聊天会按设置提交最近若干条消息，并允许一条消息快速 `@` 多只自选股。DeepSeek 会先读取股票数据目录，再按模型选择加载明细。`ai-t-advice` 默认编译但需用户主动启用。
...
| 7.7.1       | 短期行情 AI 上下文聚焦日 K、新闻、公告和筹码数据，补充相关纯逻辑测试并同步文档口径                                                                             |
| 7.8.0       | 财报总结、AI 短期/长期分析和做 T 参考统一接入底部状态栏持久通知并支持点击直达；财报库移除摘要/英文版过滤器，资金流向移至分时后                                 |
| 12.0.0      | 新增跟随系统、浅色和夜间三种持久化界面主题，覆盖主窗口、辅助窗口、弹窗、抽屉、表格与全部行情图表                                                               |
| 13.9.0      | 交易管理新增分红和缴税现金流水录入、展示与手工记录删除，并将超买超卖拆出的底仓记录命名为“混合底仓流水”                                                         |
| 13.9.1      | 修复 HKEXnews JSONP 尾部换行解析失败，并为 SEC EDGAR 请求增加全局限速与 403 频率受限提示                                                                       |
| 13.9.2      | 调整 SEC EDGAR 声明式请求头为兼容格式，并在公司行动中保留真实远程错误细节                                                                                      |
| 13.10.0     | AI 助手新增智普 GLM、Kimi、MiniMax、腾讯混元、文心一言、阿里千问、小米 MiMo、Grok、Gemini 和 Anthropic，并支持通过 API Key 获取模型列表                         |
