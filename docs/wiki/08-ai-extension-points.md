...
- “短期行情”从市场观察快照中只提取日 K 技术/趋势/动量/波动指标、新闻、公告事件和最后一次筹码分布；分时、VWAP、开盘区间、盘口、日内资金流和即时相对强弱不会发送给模型，也不读取基本面筛选、财务质量或分红融资结果。
- “长期价值”读取五年 ROE/ROIC、利润、现金流、自由现金流、负债、净负债、基本面质量与风险、用户已生成的管理层讨论/审计意见/重要附注/AI 结论摘要、应用统一口径的简化 DCF、分红融资、当前 PE/PB/PCF、近五年历史分位、快照日行业分位、PCF/PE剪刀差、总市值与流通市值，以及日 K 计算的 20/60/120/250 日收益、均线距离和 250 日价格位置；不读取财报 PDF 原文、分时、盘口、资金流、短线技术指标、筹码或即时新闻。没有本地财报总结时直接忽略，不列为数据缺失。
- 银行、保险和券商的普通企业 PCF、ROIC、自由现金流和净负债在上下文中置为不适用，并要求模型声明缺少金融行业专用指标。长期结果固定为企业质量、财务安全、当前价格和结论四部分，结论中的长期价值与当前时机必须分别评级。
- 长期提示词要求经营质量、估值和价格时机分别判断。价格偏弱可以改善时机评价，但不能反向改变盈利质量、现金质量或资本效率结论；缺少行业/历史估值分位时也不能断言绝对低估。DCF 可用时必须引用每股模型估值、相对现价幅度、DCF/现价和现价/DCF，并声明模型假设与敏感性；DCF/现价低于70%时必须列入当前价格结论与风险，不得被低 PE/PB 抵消。DCF 不可用时模型不得自行换假设重算。
...
AI 助手的“券商导入”页支持粘贴文字、TXT、XLSX 和图片，统一提取三类已经发生的流水：买卖成交、税前现金分红和红利税。TXT 与 XLSX 先由主进程在本地读取，XLSX 不执行公式、宏或外部链接；图片能力同时按 Provider 适配器和当前模型 ID 判断，不支持图片理解的模型不能选择或拖入图片，但仍可导入文字、TXT 和 XLSX。

模型只生成可编辑草稿，不能直接修改持仓。主进程重新以当前自选列表匹配股票，校验发生时间、价格、数量、实际费用、币种/汇率、来源位置、券商编号和重复记录，并用统一组合账本预演导入后的持仓与成本。用户确认后，选中流水在同一个全局 revision 中原子提交：成交写为 `source: brokerImport` 的底仓交易，现金分红与红利税写入对应现金账本类型，不自动归入 T 批次。导入来源哈希、文件名、行号/图片位置和券商编号随账本保留，用于再次导入时幂等检查；原始附件不持久化。
...
模型返回结构化的观望、正 T 或反 T 参考。主进程再校验价格、方向、持仓上限和 100 股整数倍；模块保持只读，不修改 T 计划、不创建交易或自动下单。用户需要在“做 T 管理”中自行复核并手动录入。
...
- AI 分析结果明确展示生成时间、快照时间和不确定性；券商流水识别结果必须经过用户复核和本地账本校验，模型输出不能直接替代或覆盖真实持仓。
