...
- 市盈率 TTM、市净率、自由现金流、ROIC和净负债；PE/PB跟随行情刷新，后三项使用最新完整财年快照。
...
- “长期价值”读取五年 ROE/ROIC、利润、现金流、自由现金流、负债、净负债、基本面质量与风险、分红融资、当前 PE/PB、近五年历史分位、快照日行业分位，以及日 K 计算的 20/60/120/250 日收益、均线距离和 250 日价格位置；不读取分时、盘口、资金流、短线技术指标、筹码或即时新闻。
- 银行、保险和券商的普通企业 ROIC、自由现金流和净负债在上下文中置为不适用，并要求模型声明缺少金融行业专用指标。长期结果固定为企业质量、财务安全、当前价格和结论四部分，结论中的长期价值与当前时机必须分别评级。
- 长期提示词要求经营质量、估值和价格时机分别判断。价格偏弱可以改善时机评价，但不能反向改变盈利质量、现金质量或资本效率结论；缺少行业/历史估值分位时也不能断言绝对低估。
