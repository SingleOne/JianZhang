...
- 持有筹码分布、周/月 K 的 BOLL 开关和日 K 叠加指标选择，并按图表内操作持久化到 `AppSettings`。
- 条件加载全局 AI 助手抽屉。
- 配置导入导出、手动刷新、交易日历刷新。
- 计算组合收益和大盘指数摘要。
...
独立的全市场基本面初筛弹窗。“当前筛选”默认只显示普通企业中同时通过五年 ROE、现金利润质量和同行业杠杆三项条件的公司，也可以关闭“仅显示全部通过”查看未入选证据。支持调整 ROE 口径和门槛、累计/当年现金转换率、行业负债百分位，按指标排序、分页、展开五年财务明细，并与自选股添加和定位联动。高级筛选以完整名称展示七类固定质量标签和八类固定风险提示，两组条件内部均为“同时满足”，并与当前硬筛选组合；新增“现金含金量”“PCF高于PE”“多年剪刀差”用于识别利润与经营现金流的匹配程度。标签计数基于全市场固定口径，不随临时规则变化。风险按红色严重风险和橙色关注项区分。金融企业只展示，不参与三项规则、质量标签和风险提示。
...
顶部“追踪复盘”打开集中档案弹窗，可按状态、来源和关键词过滤，查看仍在自选或已经移除的历史股票。主表“追踪”标记可直接打开个股详情“选股追踪”页，`StockTrackingEditor` 使用组件内草稿编辑标签、选股逻辑、时间线和停止结论，点击保存后才写入完整 `AppState`。其中选股逻辑与新增追踪记录采用左右双栏布局；`StockTrackingMetricsPanel` 展示当前六类量价状态、最新 5/10/20 日量比和实时量比，可切换按需加载的 `StockTrackingPriceVolumeChart`、`StockTrackingMetricsChart` 与 `StockTrackingRealtimeVolumeRatioChart`：前者绘制收盘价、MA5/10/20、成交量、均量和状态/背离标记，中者绘制三周期日量比和 1.00 倍基准线，后者绘制当日实时量比、累计成交量和交易进度预期量。停止追踪只改变状态并移出“追踪”分组；重新追踪沿用原档案、指标快照并追加系统时间线。
...
- 最新价
- 涨跌幅（下方同时显示板块涨跌幅）
- 添加以来（涨跌幅与每股涨跌额；旧数据未记录添加基准价时显示为空）
- 分红融资比（下方按前一日收盘价显示最近完整年度股息率）
...
- 管理分时、资金流、分红融资、基本面、财报库、市场观察、五日、日/周/月 K、板块、AI 分析和 AI 做 T 参考标签；资金流标签紧跟分时，AI 分析和做 T 参考位于标签栏最后，可选模块标签按构建开关出现。
- 基本面页展示默认三项规则的结论和阈值证据、简化 DCF 每股估值及相对实时股价的高低幅度、DCF/现价和现价/DCF双向比值、与市场观察复用的“估值与资本回报”区域、七类优质标签、八类风险关注、同一行业普通企业中的 ROE/现金质量/低负债名次、五年财务明细、快照生成时间与过期原因；首个结论块右侧的“基本面怎么看”可展开结合当前五年数据的阅读指南。DCF/现价低于70%时显示醒目文字提醒，计算假设直接列在结果下方。质量和风险标签都给出计算证据，风险使用完整名称并按红色严重风险、橙色关注项分级。主表“基本/待核/缺数/金融/风险/关注”标签可直接展开到此页，悬停说明具体状态或风险项。
- 财报库页查询当前股票最近五个报告年度的巨潮定期报告，不提供搜索、财年、报告类型、摘要或英文版筛选，直接按报告所属财年分组，只展示全文并可打开官方 PDF。条目小字显示已保存总结，没有总结时可一键调用当前 AI 模型生成并保存。
- 财报总结、AI 短期行情、AI 长期价值和做 T 参考生成成功后，统一向 `App.tsx` 发送完成事件。主窗口在底部状态栏“最近更新”旁保留最新通知和待处理数量，不设置自动消失计时；点击后清除主表筛选，将对应股票滚动到顶部吸顶位置，展开详情并切换到目标标签或 AI 子页，同时清除该条通知，退出软件后内存通知自然清空。
- “基本面”和“市场观察”复用同一“估值与资本回报”区域，展示实时 PE TTM/PB、当前 PCF TTM、三项估值的近五年历史分位、快照日行业分位、PCF/PE剪刀差、总市值、流通市值，以及最新自由现金流、ROIC和净负债；行情、财报、历史区间及行业样本日期分别标注。银行、保险和券商的普通企业 PCF、自由现金流、ROIC和净负债显示“不适用”。
- AI 分析页内部提供“短期行情”和“长期价值”两个独立入口。短期按日 K 尺度组合技术指标、新闻、公告事件和筹码，明确排除分时、盘口等瞬时数据；长期组合五年基本面、已有的结构化财报 AI 总结、分红融资、估值分位和长期价格强弱，财报总结缺失时忽略，PDF 原文不进入上下文。结果固定展示“企业质量、财务安全、当前价格、结论”，结论再拆为长期价值与当前时机。两类结果分别缓存，旧版 AI 结果继续作为短期行情结果读取。
- 为各价格周期维护数据、错误和加载状态。
...
- 日 K 通过共享 `calculateMovingAverages` 叠加 MA5/MA10/MA20/MA60，或切换为 `calculateBollingerBands` 生成的 BOLL(20,2) 三轨线；下方 `MA｜BOLL｜关闭` 指标栏随十字光标显示对应数值并持久化选择。周/月 K 保留原 BOLL 开关。

### `ChipDistributionPanel.tsx`

- 接收当前日 K 选中范围，在 renderer 内计算价格筹码桶、平均成本、获利比例和 70%/90% 成本区间，并显示最高峰价位与现价上方第一个局部峰值；鼠标移入图表或聚焦峰值单元格后，同步在左侧坐标区显示价格和筹码占比，对应柱条放大并以获利红/套牢绿产生同色脉动阴影。
...
- 日 K 均线 MA5 为橙色、MA10 为蓝色、MA20 为紫色、MA60 为青色。
