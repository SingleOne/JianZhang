...
- 基本面页展示默认三项规则的结论和阈值证据、六类质量特征、六类风险关注、同一行业普通企业中的 ROE/现金质量/低负债名次、五年财务明细、快照生成时间与过期原因；质量和风险标签都给出计算证据，风险使用完整名称并按红色严重风险、橙色关注项分级。主表“基本/待核/缺数/金融/风险/关注”标签可直接展开到此页，悬停说明具体状态或风险项。
- 市场观察页顶部的“估值与资本回报”区域展示随主行情更新的市盈率 TTM、市净率，以及 schema v2 基本面快照中的最新自由现金流、ROIC和净负债；使用旧快照时对应财务项显示待更新，基本面过期时保留数值并提示原因。
- AI 分析页内部提供“短期行情”和“长期价值”两个独立入口。短期只读取市场观察指标、新闻、事件和筹码；长期组合五年基本面、分红融资、当前 PE/PB和长期价格强弱，并把经营质量、现金质量、资本效率、财务结构、估值、股东回报和价格时机分区展示。两类结果分别缓存，旧版 AI 结果继续作为短期行情结果读取。
- 为各价格周期维护数据、错误和加载状态。
