...
缺少批次字段表示批次外的底仓交易；带 `batchId` 的记录由 `getBatchTrades` 按需投影给活动批次或历史批次。批次对象本身不再持有 `trades` 数组。
...
- 修改已有持仓的数量、成本价或建仓日期时，会生成一条“持仓校准”账本记录，并与买卖流水一起显示在交易记录中。
- 删除持仓校准或其他较早记录时，如果其后仍有交易记录，确认弹窗会说明级联影响；确认后同步删除该记录及之后的记录，并重新计算持仓、收益和相关 T 批次。

## 账本计算

### 超买、超卖时拆分记录

正 T 卖出超过剩余 T 仓，或反 T 买入超过待回补数量时，交易管理会按所选用途一次保存两条独立记录：

- 第一条只记录完成当前 T 批次所需的数量。
- 选择“增加底仓 / 减持底仓”，第二条作为账户级“混合底仓流水”显示在底仓流水中。
- 选择“开启正 T / 反 T 批次”，第一条结束原批次，第二条作为新反向批次的首笔成交；新批次以成交时的完整账本持仓为起点。
- 数量恰好完成当前批次时，只保存一条记录。

拆分记录保留相同的成交时间、价格及 `splitSource` 来源标识。来源仅用于显示“同笔成交拆分”并追溯原成交，不参与持仓和收益计算。原整笔成交不会作为第三条记录保留。

费用按整笔计算一次，以分为单位按数量比例分配到两条记录；各费用项目和总费用均保持守恒。编辑拆分记录时沿用已保存的分摊费用，需要重新计算时由用户明确选择。

两条记录各有独立 ID，可分别编辑或删除，并经过批次与完整账本校验。删除某条拆分记录不会因共同来源而删除另一条或后续记录；已结算批次仍需保持 T 仓买卖平衡。两条记录、批次切换和持仓更新一次提交，校验失败时不保存。

自动拆分只在保存成交时执行，历史记录保持原样。

### 分红与缴税流水

交易管理提供“分红与缴税”录入。分红保存为 `cashDividend` 账本记录，填写税前总额；登记股数可选，填写后自动计算每股分红。缴税保存为 `withholdingTax` 账本记录，并按现金流出计入收益统计。两类流水都不改变持仓数量和持仓成本。

交易管理同时显示手工录入、公司行动和券商导入的分红与缴税流水。手工录入的记录可以直接删除；公司行动产生的记录仍从公司行动入口撤销，避免公司行动状态与账本不一致。
...
结算表单默认带入当前摊薄持仓成本。摊薄成本不可用时，最终收益回退为批次流水收益。
...
“当前展示持仓收益，不是账户收益”固定显示在弹窗标题栏。顶部同时展示当前周期收益小计和累计收益小计；其余收益分项保持当前或最近持仓周期口径。用户可以按股票录入券商显示的人民币持仓收益，系统保存其与当前周期账本计算值的差额，并把该调整随当前周期计入累计收益。原币收益与价格/汇率归因仍保留账本计算值，人民币收益会明确标示调整额。

股票行同时显示当前周期收益和累计收益。当前周期从最近一次零仓重新开仓或收益基准重置开始；累计收益包含全部持仓周期，但遇到明确的收益基准重置时只累计重置后的部分。操作列按上下两行提供“重新计算”和“周期明细”，周期明细弹窗展示累计总计以及每个持仓周期的起止日期、状态、收益分类和完整性。重置前周期保留展示并标记为不计入当前累计。
