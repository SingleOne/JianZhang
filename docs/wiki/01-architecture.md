...
| 模块                                         | 职责                                                            |
| -------------------------------------------- | --------------------------------------------------------------- |
| `StateStore`                                 | 加载/规范化状态、原子保存、最近可用备份和损坏恢复               |
...
sequenceDiagram
    ...
    E->>S: StateStore.load + normalize/recover
...
| 基本面财务数据                                          | `FundamentalDataService` 兼容读取 schema v1-v7 用户快照；v7增加 PCF TTM 及行业分位，缺失时首次自动获取，旧版、过期或完整财年落后只提示 | 是，`fundamentals/snapshot.json`、`diagnostics.json`                                 |
...
渲染层保存状态时会先本地更新，再调用 `state:save`。主进程会执行 normalize，持久化后通过 `state:updated` 向所有窗口广播规范化结果。
...
交易数据结构变更还要确认统一账本、`tradeRecords` 镜像和批次关联同步更新。
