...
> 文档状态：基础 AI 与独立做 T 参考均已实施；AI 能力按个人自用场景默认可用，所有模型调用仍由用户主动触发
>
> 编写日期：2026-07-21
>
> 当前应用版本：`4.9.0`
>
> 本文是 AI 接入的当前实施依据；如与早期 [AI 可移除模块设计](wiki/08-ai-extension-points.md) 冲突，以本文为准。2026-07-21 已完成基础 AI 和 `ai-t-advice` 的独立模块实现；2026-07-23 调整为个人自用产品边界，不再因合规风险限制 AI 输出做 T 思路。做 T 参考默认进入构建产物并默认可用，仍保留单独构建剔除和源码删除能力作为工程维护手段。

## 1. 目标与结论

在不侵入见涨核心行情、持仓和现有做 T 功能的前提下，增加两层 AI 能力：
...
1. **AI 基础模块 `ai`**
   ...
   - 在用户主动询问时，可以基于已附带的股票快照讨论做 T 思路，但不直接写入 T 计划。
2. **做 T 参考模块 `ai-t-advice`**
   - 独立于 AI 基础模块，默认编译进应用并默认可用。
   - 负责结构化输出交易方向、价格区间、数量和失效条件的做 T 参考。
   - 提供本地 Schema、价格、持仓和数量校验，以及应用到现有 T 计划前的差异预览。

现有 `market-insight` 模块继续负责确定性指标、新闻聚合和客观观察事件。AI 只消费其公开快照，不重新计算指标，也不反向控制它。

> 私用边界：本功能仅按个人自用场景设计，不再把“合规评估是否通过”作为功能入口、提示词或输出内容的限制条件。模块化、构建开关和源码删除能力仍保留，用于维护、调试、性能控制和发行策略。
...
- 做 T 参考在基础 AI 启用时默认编译；显式设置 `JIANZHANG_AI_T_ADVICE_MODULE=0` 时单独剔除。
