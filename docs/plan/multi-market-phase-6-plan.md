...
- 建立版本化统一组合账本，历史成交自动迁移并继续维护兼容镜像；持仓、成本、现金收益、修订差额和撤销均由账本计算。
- 接入 HKEXnews 港股公司行动候选和 SEC EDGAR 美股披露候选；候选缓存 24 小时，后台检查与行情开市刷新解耦，官方正文仅在影响预览时按需下载。
- 支持现金分红、资本返还、送股、拆合股、供股认购/不参与、代码变更、证券转换、退市现金和手工现金调整；所有事件必须先预览再确认。
- 增加个股公司行动时间线和全局待确认中心，支持官方原文、筛选、忽略、修订差额、撤销记录及券商实际费用/汇率补录。
- 已确认、已忽略、已入账状态和组合账本作为用户数据进入本地状态、备份及 Gist 恢复；候选和官方文档仍是可重建缓存，可单独清理。
- 新增公司行动提取和统一账本单元测试；按项目约定仅执行类型检查、ESLint 与差异静态检查，未主动运行测试、界面验证或构建打包。
