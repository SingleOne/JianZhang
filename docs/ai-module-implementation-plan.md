...
> 文档状态：基础 AI 与独立做 T 参考均已实施；做 T 参考默认编译、运行时默认关闭
>
> 编写日期：2026-07-21
>
> 当前应用版本：`4.1.0`
>
> 本文是 AI 接入的当前实施依据；如与早期 [AI 可移除模块设计](wiki/08-ai-extension-points.md) 冲突，以本文为准。2026-07-21 已完成基础 AI 和 `ai-t-advice` 的独立模块实现；做 T 参考默认进入构建产物，但运行时仍默认关闭，并继续保留单独构建剔除和源码删除能力。
