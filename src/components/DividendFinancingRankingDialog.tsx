...
function StockAction({ item, watchedStock, addingCode, onAdd, onView }: StockActionProps) {
  return watchedStock ? (
    <button
  ...
  return (
    <div className="dividend-change-panel">
      ...
      <div className="dividend-change-summary">
        <div><span>新入榜</span><strong>{report.summary.addedCount}</strong></div>
        <div><span>移出榜单</span><strong>{report.summary.removedCount}</strong></div>
        <div><span>排名变化</span><strong>{report.summary.rankChangedCount}</strong></div>
        <div><span>分红增加</span><strong>{report.summary.dividendIncreasedCount}</strong></div>
        <div className="is-risk"><span>融资增加</span><strong>{report.summary.financingIncreasedCount}</strong></div>
      </div>
              ...
              <tr key={item.code}>
                <td>
                  <div className="dividend-ranking-stock">
                    ...
                    <span>{item.code} · {MARKET_LABELS[item.market]}</span>
                  </div>
                </td>
                <td>
                  <div className="dividend-change-tags">
                    {item.changeTypes.map((type) => (
                      <span className={`is-${type}`} key={type}>{changeLabels[type]}</span>
                    ))}
                  </div>
                </td>
                <td className={signedClass(item.rankChange ?? 0)}>
                  {item.previousRank ?? '--'} → {item.currentRank ?? '--'}
                  {item.rankChange ? <small>（{item.rankChange > 0 ? '+' : ''}{item.rankChange}）</small> : null}
                </td>
                <td className={signedClass(item.ratioChange ?? 0)}>
                  {item.previousRatio?.toFixed(2) ?? '--'}% → {item.currentRatio?.toFixed(2) ?? '--'}%
                </td>
                <td className={signedClass(item.dividendIncreaseYi)}>
                  {item.dividendIncreaseYi > 0 ? `+${formatAmount(item.dividendIncreaseYi)} 亿元` : '--'}
                </td>
                <td className={item.financingIncreaseYi > 0 ? 'is-negative' : 'is-zero'}>
                  {item.financingIncreaseYi > 0 ? `+${formatAmount(item.financingIncreaseYi)} 亿元` : '--'}
                </td>
                <td className={signedClass(item.qualityScoreChange ?? 0)}>
                  {item.previousQualityScore?.toFixed(1) ?? '--'} → {item.currentQualityScore?.toFixed(1) ?? '--'}
                </td>
              </tr>
            ))}
          </tbody>
  ...
  return (
    <div className="dividend-visualization-panel">
      <div className="dividend-scatter-intro">
        <span>横轴累计融资、纵轴累计分红，均为对数刻度；点越靠左上，历史股东现金回报特征越强。</span>
        <div className="dividend-scatter-legend">
          {(Object.keys(MARKET_LABELS) as DividendFinancingMarket[]).map((market) => (
            <span className={`is-${market.toLowerCase()}`} key={market}>{MARKET_LABELS[market]}</span>
          ))}
        </div>
      </div>
      <div className="dividend-scatter-layout">
        ...
        >
          {ticks.map((position) => {
            ...
            return (
              <g key={position}>
                <line className="dividend-scatter-grid" x1={xForLog(xLog)} x2={xForLog(xLog)} y1={margin.top} y2={height - margin.bottom} />
                <text className="dividend-scatter-axis-label" x={xForLog(xLog)} y={height - 28} textAnchor="middle">
                  {formatAmount(10 ** xLog)}
                </text>
                <line className="dividend-scatter-grid" x1={margin.left} x2={width - margin.right} y1={yForLog(yLog)} y2={yForLog(yLog)} />
                <text className="dividend-scatter-axis-label" x={margin.left - 10} y={yForLog(yLog) + 4} textAnchor="end">
          ...
          <text className="dividend-scatter-title-label" x={width / 2} y={height - 5} textAnchor="middle">累计A股融资（亿元，对数）</text>
          <text className="dividend-scatter-title-label" transform={`translate(18 ${height / 2}) rotate(-90)`} textAnchor="middle">累计A股分红（亿元，对数）</text>
          {drawableRows.map((item) => (
            ...
            >
              <title>{item.name} {item.code} · 分红 {formatAmount(item.dividendYi)}亿 · 融资 {formatAmount(item.financingYi)}亿</title>
            </circle>
          ))}
        </svg>
        {selected ? (
          <aside className="dividend-scatter-selection">
            <div>
              <span>{selected.code} · {MARKET_LABELS[selected.market]}</span>
              <strong>{selected.name}</strong>
            </div>
            <dl>
              <div><dt>分红融资比</dt><dd>{selected.ratio.toFixed(2)}%</dd></div>
              <div><dt>累计分红</dt><dd>{formatAmount(selected.dividendYi)} 亿元</dd></div>
              <div><dt>累计融资</dt><dd>{formatAmount(selected.financingYi)} 亿元</dd></div>
              <div><dt>净回报额</dt><dd className={signedClass(netReturn(selected))}>{formatAmount(netReturn(selected))} 亿元</dd></div>
              <div><dt>连续分红</dt><dd>{selected.consecutiveDividendYears ?? '--'} 年</dd></div>
              <div><dt>质量评分</dt><dd>{selected.qualityScore?.toFixed(1) ?? '--'} / 100</dd></div>
            </dl>
            <StockAction
              item={selected}
...
export function DividendFinancingRankingDialog({
  ...
  const filteredRows = useMemo(() => {
    ...
    return rows.sort((left, right) => {
      const leftValue = sortKey === 'netReturnYi'
        ? netReturn(left)
        : (left[sortKey] ?? 0)
      const rightValue = sortKey === 'netReturnYi'
        ? netReturn(right)
        : (right[sortKey] ?? 0)
  ...
  const addStock = async (item: DividendFinancingRankingItem) => {
    ...
    try {
      ...
      onAddStock(result)
      setActionMessage(`${result.name}已加入自选`)
    } catch (reason) {
    ...
    <div className="dividend-ranking-backdrop" role="presentation" onMouseDown={onClose}>
      ...
      >
        <header className="dividend-ranking-header">
          <div className="dividend-ranking-title-icon" aria-hidden="true"><Trophy size={21} /></div>
        ...
        {loading ? (
          ...
          <>
            ...
            <div className="dividend-ranking-summary">
              ...
              {tierCounts.map((item) => (
                <div key={item.value}><span>{item.value}% 以上</span><strong>{item.count}</strong></div>
              ))}
              <details>
                <summary><Database size={14} />统计口径与评分</summary>
                <div className="dividend-ranking-methodology">
                  <p>净回报额 = 上市以来累计A股现金分红 − 累计A股股权融资；分红融资比 = 分红 ÷ 融资 × 100%。</p>
                  <p>年度分红按已实施事件拆分，再按精确累计分红总额等比例校准；融资只统计IPO、增发和配股的募集净额。</p>
                  <p>回报质量评分：比例分位30分、净回报分位25分、连续性25分、近期增长10分、融资纪律10分，只在当前“超过100%”样本内比较。</p>
                  <p>融资额很小会放大比例，请结合“最低融资额”、净回报额和累计分红筛选，避免只看极端比例。</p>
                  <p>识别有效股票 {snapshot.activeStockCount} 只，精确复核 {snapshot.exactCandidateCount} 只；接口失败：融资 {snapshot.financingErrorCount} 只、分红 {snapshot.dividendErrorCount} 只。数据不构成投资建议。</p>
                </div>
              </details>
            </div>
            ...
            <div className="dividend-ranking-tabs">
              <div role="tablist" aria-label="分红融资分析类型">
                <button className={viewMode === 'ranking' ? 'is-active' : ''} type="button" role="tab" aria-selected={viewMode === 'ranking'} onClick={() => setViewMode('ranking')}>深度榜单</button>
                <button className={viewMode === 'changes' ? 'is-active' : ''} type="button" role="tab" aria-selected={viewMode === 'changes'} onClick={() => setViewMode('changes')}>更新变化</button>
                <button className={viewMode === 'visualization' ? 'is-active' : ''} type="button" role="tab" aria-selected={viewMode === 'visualization'} onClick={() => setViewMode('visualization')}>可视化选股</button>
              </div>
              <button
              ...
              <div className="dividend-ranking-filters">
                <div className="dividend-ranking-toolbar">
                  <label className="dividend-ranking-search">
                    <Search size={16} />
                    <input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="搜索股票名称或代码" aria-label="搜索分红融资榜" autoFocus />
                    {query ? <button type="button" onClick={() => setQuery('')} aria-label="清空搜索"><X size={14} /></button> : null}
                  </label>
                  <label>比例<select value={threshold} onChange={(event) => setThreshold(Number(event.target.value))}>
                    <option value={100}>100% 以上</option><option value={200}>200% 以上</option><option value={500}>500% 以上</option><option value={1000}>1000% 以上</option>
                  </select></label>
                  <label>市场<select value={market} onChange={(event) => setMarket(event.target.value as MarketFilter)}>
                    <option value="ALL">全部市场</option><option value="SH">沪市</option><option value="SZ">深市</option><option value="BJ">北交所</option>
                  </select></label>
                  <label className="dividend-ranking-watchlist-filter"><input type="checkbox" checked={onlyWatchlist} onChange={(event) => setOnlyWatchlist(event.target.checked)} />仅看自选</label>
                  <span className="dividend-ranking-result-count">当前 {filteredRows.length} 只</span>
                </div>
                <div className="dividend-ranking-deep-filters">
                  <span>深度条件</span>
                  <label>净回报 ≥ <input type="number" min="0" step="1" value={minNetReturn} onChange={(event) => setMinNetReturn(Number(event.target.value))} /> 亿</label>
                  <label>累计分红 ≥ <input type="number" min="0" step="1" value={minDividend} onChange={(event) => setMinDividend(Number(event.target.value))} /> 亿</label>
                  <label title="适当提高此值可排除小融资分母造成的极端比例">累计融资 ≥ <input type="number" min="0" step="1" value={minFinancing} onChange={(event) => setMinFinancing(Number(event.target.value))} /> 亿</label>
                  <label>连续分红 ≥ <input type="number" min="0" step="1" value={minConsecutiveYears} onChange={(event) => setMinConsecutiveYears(Number(event.target.value))} /> 年</label>
                  <label>质量评分 ≥ <input type="number" min="0" max="100" step="1" value={minQualityScore} onChange={(event) => setMinQualityScore(Number(event.target.value))} /> 分</label>
                </div>
              </div>
            ) : null}
            ...
            {viewMode === 'visualization' ? (
              <VisualizationPanel rows={filteredRows} watchlistByCode={watchlistByCode} addingCode={addingCode} onAdd={(item) => void addStock(item)} onView={onViewStock} />
            ) : null}
            {viewMode === 'ranking' ? (
              <>
                <div className="dividend-ranking-table-scroller">
                  <table className="dividend-ranking-table">
                    <thead><tr>
                      <th><button type="button" onClick={() => changeSort('rank')}>排名{sortIndicator('rank')}</button></th>
                      <th>股票</th>
                      <th><button type="button" onClick={() => changeSort('dividendYi')}>累计分红{sortIndicator('dividendYi')}</button></th>
                      <th><button type="button" onClick={() => changeSort('financingYi')}>累计融资{sortIndicator('financingYi')}</button></th>
                      <th><button type="button" onClick={() => changeSort('netReturnYi')}>净回报额{sortIndicator('netReturnYi')}</button></th>
                      <th><button type="button" onClick={() => changeSort('ratio')}>分红融资比{sortIndicator('ratio')}</button></th>
                      <th><button type="button" onClick={() => changeSort('recent5YearDividendYi')}>近5年分红{sortIndicator('recent5YearDividendYi')}</button></th>
                      <th><button type="button" onClick={() => changeSort('consecutiveDividendYears')}>连续分红{sortIndicator('consecutiveDividendYears')}</button></th>
                      <th><button type="button" onClick={() => changeSort('qualityScore')}>质量评分{sortIndicator('qualityScore')}</button></th>
                      <th>操作</th>
                    </tr></thead>
                    <tbody>
                      {visibleRows.map((item) => (
                        <tr key={item.code}>
                          ...
                          <td><div className="dividend-ranking-stock"><strong>{item.name}</strong><span>{item.code}<em>{MARKET_LABELS[item.market]}</em></span></div></td>
                          <td className="dividend-ranking-number">{formatAmount(item.dividendYi)}<small>亿元</small></td>
                          <td className="dividend-ranking-number">{formatAmount(item.financingYi)}<small>亿元</small></td>
                          <td className={`dividend-ranking-number dividend-return-value ${signedClass(netReturn(item))}`}>{formatAmount(netReturn(item))}<small>亿元</small></td>
                          <td><strong className={`dividend-ranking-ratio ${ratioTier(item.ratio)}`}>{item.ratio.toFixed(2)}%</strong></td>
                          <td className="dividend-ranking-number">{item.recent5YearDividendYi === undefined ? '--' : formatAmount(item.recent5YearDividendYi)}<small>{item.recent5YearDividendYi === undefined ? '' : '亿元'}</small></td>
                          <td className="dividend-ranking-number">{item.consecutiveDividendYears ?? '--'}<small>{item.consecutiveDividendYears === undefined ? '' : '年'}</small></td>
                          <td>
                            {item.qualityScoreBreakdown ? (
                              <details className="dividend-quality-score">
                                <summary><strong>{item.qualityScore?.toFixed(1)}</strong><span>第 {item.scoreRank} 名</span></summary>
                                <div>{scorePartRows(item.qualityScoreBreakdown).map(([label, value, maximum]) => <p key={label}><span>{label}</span><strong>{value.toFixed(1)} / {maximum}</strong></p>)}</div>
                              </details>
                            ) : '--'}
                          </td>
                          <td><StockAction item={item} watchedStock={watchlistByCode.get(item.code)} addingCode={addingCode} onAdd={(stock) => void addStock(stock)} onView={onViewStock} /></td>
                        </tr>
                      ))}
                      {visibleRows.length === 0 ? <tr><td className="dividend-ranking-empty" colSpan={10}>当前筛选条件下没有股票</td></tr> : null}
                    </tbody>
                  </table>
                </div>
                <footer className="dividend-ranking-footer">
                  ...
                  <div>
                    <span>{filteredRows.length === 0 ? '0 条' : `${(currentPage - 1) * PAGE_SIZE + 1}–${Math.min(currentPage * PAGE_SIZE, filteredRows.length)} / ${filteredRows.length}`}</span>
                    ...
                    <button type="button" onClick={() => setPage((value) => Math.min(pageCount, value + 1))} disabled={currentPage === pageCount} aria-label="下一页"><ChevronRight size={16} /></button>
                  </div>
                </footer>
              </>
            ) : null}
