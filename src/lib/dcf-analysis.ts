...
export function createDcfAnalysis(
  ...
  const historicalGrowthRate = freeCashFlows.length >= 2
  ...
  const terminalValue = finalForecastCashFlow * (1 + terminalGrowthRate)
    / (discountRate - terminalGrowthRate)
  const enterpriseValue = discountedForecastCashFlow
    + terminalValue / Math.pow(1 + discountRate, DCF_FORECAST_YEARS)
  const equityValue = enterpriseValue - netDebt
  const sharesOutstanding = totalMarketValue / closePrice
  const fairValuePerShare = equityValue / sharesOutstanding
  const comparableCurrentPrice = currentPrice !== null
    && currentPrice !== undefined
    && currentPrice > 0
    ? currentPrice
    : null
  const fairValueToPricePercent = comparableCurrentPrice === null
    ? null
    : fairValuePerShare / comparableCurrentPrice * 100
