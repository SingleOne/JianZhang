...
describe('portfolio calculations', () => {
  ...
  it('uses current holding-cycle profit to calculate diluted cost and return rate', () => {
    const metrics = calculatePositionMetrics(
    ...
    expect(metrics.holdingCostBasis).toBeCloseTo(1020.47)
    expect(metrics.holdingCost).toBeCloseTo(10.2047)
    expect(metrics.profitPercent).toBeCloseTo(-29.8362)
    expect(metrics.cnyCostBasis).toBeCloseTo(744.725)
    expect(metrics.cnyHoldingCostBasis).toBeCloseTo(1020.47)
    expect(metrics.cnyProfitPercent).toBeCloseTo(-29.8362)
  })
