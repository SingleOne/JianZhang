...
export function detectWatchEvents(
  ...
  if (volumeRatio !== null && volumeRatio >= settings.volumeSpikeRatio) {
    activeContinuousFingerprints.add(
      eventFingerprint(current.quoteId, 'volume_spike', '5m-volume-ratio')
    )
    events.push(
      draft(
        current.quoteId,
        ...
        'attention',
        '5 分钟成交量显著放大',
        [`当前为近 20 个窗口中位数的 ${volumeRatio.toFixed(2)} 倍`],
        occurredAt,
        20,
        [],
        '5m-volume-ratio'
  ...
  if (position !== null && (position >= 95 || position <= 5)) {
    ...
    activeContinuousFingerprints.add(
      eventFingerprint(current.quoteId, 'intraday_extreme', extremeKey)
    )
    events.push(
      draft(
        current.quoteId,
        ...
        'info',
        '价格接近当日区间极值',
        [`当前位于当日高低区间 ${position.toFixed(1)}% 位置`],
        occurredAt,
        20,
        [],
        extremeKey
  ...
  const previousPrice =
    previous && previous.chartOverlay.vwap !== null
  ...
  if (
    ...
    previousPrice !== null &&
    rangeHigh !== null &&
    previousPrice <= rangeHigh &&
    inferredPrice > rangeHigh
  ) {
    events.push(
      draft(
        current.quoteId,
  ...
  if (inferredPrice !== null && rangeHigh !== null && inferredPrice > rangeHigh) {
    activeContinuousFingerprints.add(
      eventFingerprint(current.quoteId, 'opening_range_break', 'above-opening-range')
    )
  }
  if (
    inferredPrice !== null &&
    previousPrice !== null &&
    rangeLow !== null &&
    previousPrice >= rangeLow &&
    inferredPrice < rangeLow
  ) {
    events.push(
      draft(
        current.quoteId,
  ...
  if (inferredPrice !== null && rangeLow !== null && inferredPrice < rangeLow) {
    activeContinuousFingerprints.add(
      eventFingerprint(current.quoteId, 'opening_range_break', 'below-opening-range')
    )
  }
  ...
  if (
    imbalance !== null &&
    previousImbalance !== null &&
    Math.abs(imbalance - previousImbalance) >= 0.35
  ) {
    events.push(
      draft(
        current.quoteId,
        ...
        'info',
        '五档委托不平衡发生变化',
        [`不平衡度由 ${previousImbalance.toFixed(2)} 变为 ${imbalance.toFixed(2)}`],
        occurredAt,
        15
      )
    )
  ...
  const previousRelative = previous
    ? (findValue(previous, 'relative-sector') ?? findValue(previous, 'relative-index'))
    : null
  if (
    relative !== null &&
    previousRelative !== null &&
    Math.sign(relative) !== Math.sign(previousRelative)
  ) {
    events.push(
      draft(
        current.quoteId,
