...
export function calculateTrendIndicators(inputBars: readonly KlineBar[], calculatedAt: string): IndicatorValue[] {
  ...
  const stateAgainst = (value: number | null) => latest !== null && value !== null
    ? latest === value ? 'flat' : latest > value ? 'up' : 'down'
    : 'unknown'
  const volumeWindow = bars.slice(-20).map((bar) => bar.volume)
  const currentVolume = volumeWindow.at(-1) ?? null
  const volumePercentile = currentVolume !== null && volumeWindow.length === 20
    ? volumeWindow.filter((volume) => volume <= currentVolume).length / volumeWindow.length * 100
    : null
  ...
  const turnoverPercentile = currentTurnover !== null && turnoverWindow.length === 20
    ? turnoverWindow.filter((turnover) => turnover <= currentTurnover).length / turnoverWindow.length * 100
    : null
  ...
  return [
    ...
    indicator('macd-signal', 'MACD 信号线', signal, 'price', calculatedAt, '日K', directionState(signal)),
    indicator('macd-histogram', 'MACD 柱', histogram, 'price', calculatedAt, '日K', directionState(histogram)),
    indicator('daily-volume-percentile-20', '20日成交量分位', volumePercentile, 'percent', calculatedAt, '日K', volumePercentile === null ? 'unknown' : volumePercentile > 50 ? 'up' : volumePercentile < 50 ? 'down' : 'flat'),
    indicator('daily-turnover-percentile-20', '20日换手率分位', turnoverPercentile, 'percent', calculatedAt, '日K', turnoverPercentile === null ? 'unknown' : turnoverPercentile > 50 ? 'up' : turnoverPercentile < 50 ? 'down' : 'flat')
  ]
}
