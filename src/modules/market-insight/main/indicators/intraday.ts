...
function percentageChange(previous: number | undefined, current: number | undefined): number | null {
  return previous === undefined || current === undefined || previous === 0 ? null : (current / previous - 1) * 100
}

function rangeForMinutes(bars: readonly KlineBar[], endMinute: number): { high: number | null; low: number | null } {
...
export function calculateIntradayIndicators(
  ...
  const vwapDeviation = displayLatest !== null && vwap !== null && vwap !== 0
    ? (displayLatest / vwap - 1) * 100
    : null
  ...
  const dayPosition = displayLatest !== null && high !== null && low !== null && high !== low
  ...
  const currentWindowVolume = closedBars.length >= 5
  ...
  for (let end = closedBars.length - 5; end >= 5 && previousWindowVolumes.length < 20; end -= 5) {
    previousWindowVolumes.push(closedBars.slice(end - 5, end).reduce((total, bar) => total + bar.volume, 0))
  }
  const medianWindowVolume = median(previousWindowVolumes)
  const volumeRatio = currentWindowVolume !== null && medianWindowVolume !== null && medianWindowVolume > 0
    ? currentWindowVolume / medianWindowVolume
    : null
  const recentPriceChange = percentageChange(closedBars.at(-6)?.close, last?.close)
  const volumeDirection = volumeRatio === null || recentPriceChange === null
    ? 'unknown'
  ...
  return {
    ...
    values: [
      ...
      indicator('vwap-deviation', '相对 VWAP', vwapDeviation, 'percent', calculatedAt, '分时', directionState(vwapDeviation)),
      ...priceReturns.map(({ window, value }) => indicator(`return-${window}m`, `${window} 分钟收益`, value, 'percent', calculatedAt, '分时', directionState(value))),
      indicator('intraday-position', '当日高低点位置', dayPosition, 'percent', calculatedAt, '分时', dayPosition === null ? 'unknown' : dayPosition > 50 ? 'up' : dayPosition < 50 ? 'down' : 'flat'),
      indicator('opening-range-15-high', '开盘 15 分钟高点', openingRange15.high, 'price', calculatedAt, '分时'),
      indicator('opening-range-15-low', '开盘 15 分钟低点', openingRange15.low, 'price', calculatedAt, '分时'),
      indicator('opening-range-30-high', '开盘 30 分钟高点', openingRange30.high, 'price', calculatedAt, '分时'),
      indicator('opening-range-30-low', '开盘 30 分钟低点', openingRange30.low, 'price', calculatedAt, '分时'),
      indicator('volume-ratio-5m', '5 分钟成交量倍数', volumeRatio, 'ratio', calculatedAt, '分时', volumeRatio === null ? 'unknown' : volumeRatio > 1 ? 'up' : volumeRatio < 1 ? 'down' : 'flat'),
      indicator('price-volume-state', '量价状态', volumeRatio === null || recentPriceChange === null ? null : volumeRatio, 'ratio', calculatedAt, '分时', volumeDirection)
    ]
  }
}
