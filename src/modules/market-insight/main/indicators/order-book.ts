...
export function calculateOrderBookIndicators(
  ...
  const imbalance = bidVolume !== null && askVolume !== null && bidVolume + askVolume > 0
    ? (bidVolume - askVolume) / (bidVolume + askVolume)
    : null
  const imbalanceChange = imbalance !== null && previousImbalance !== null
  ...
  return {
    ...
    values: [
      ...
      indicator('order-book-imbalance', '五档委托不平衡', imbalance, 'ratio', calculatedAt, '盘口', directionState(imbalance)),
      indicator('order-book-imbalance-change', '委托不平衡短窗变化', imbalanceChange, 'ratio', calculatedAt, '盘口短窗', directionState(imbalanceChange))
    ]
  }
}
