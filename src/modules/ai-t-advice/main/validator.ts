...
export function parseAiTAdvice(content: string, context: AiTAdviceValidationContext): AiTAdvice {
  ...
  if (!raw || typeof raw !== 'object' || Array.isArray(raw)) throw new Error('模型返回的做 T 参考格式无效')

  const record = raw as Record<string, unknown>
  const action = record.action as AiTAdviceAction
  if (!['hold', 'forward-t', 'reverse-t'].includes(action)) throw new Error('模型返回了不支持的做 T 动作')
  ...
  if (rationale.length === 0 || risks.length === 0) throw new Error('模型返回的理由或风险提示不完整')
  ...
  if (!zone || typeof zone !== 'object' || Array.isArray(zone)) throw new Error('模型没有给出有效的参考价格区间')
  ...
  if (lower === null || upper === null || lower > upper) throw new Error('模型给出的参考价格区间无效')
  ...
  if (quantity > context.maxTradableQuantity) throw new Error('模型给出的参考数量超过当前可用持仓约束')
