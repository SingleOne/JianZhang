...
- “券商导入”支持粘贴文字、TXT、XLSX 和图片，识别已成交买卖、税前现金分红和红利税。TXT/XLSX 在本地读取，图片仅由明确支持视觉输入的 OpenAI、Gemini 或 Anthropic Provider 处理。识别结果会匹配当前自选股票、校验市场数量规则、汇率、重复流水和完整账本回放，并展示导入后的持仓/成本预览；所有选中记录作为一个 revision 原子提交，成交默认作为底仓交易，不自动写入 T 批次。
