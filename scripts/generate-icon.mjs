...
for (let y = 0; y < size; y += 1) {
  for (let x = 0; x < size; x += 1) {
    if (isInsideRoundedSquare(x, y)) setPixel(x, y, 37, 99, 235)
  }
}
...
for (let y = 0; y < size; y += 1) {
  for (let x = 0; x < size; x += 1) {
    ...
    if (onLine || onArrowA || onArrowB) setPixel(x, y, 255, 255, 255)
  }
}
...
writeFileSync('build/icon.png', PNG.sync.write(png))
